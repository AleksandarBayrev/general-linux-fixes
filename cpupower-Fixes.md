# Since newer kernels already include `cpupower.service` in systemd, there is a configuration file as well: `/etc/cpupower-service.conf`
## Edit it (this is how it looks by default on Fedora Kinoite 44):
```ini
...
#EPP=
```
## Update `GOVERNOR`, `MIN_FREQ` and `MAX_FREQ` to your desires
## Run `sudo systemctl enable --now cpupower.service`
## If `tuned` service is present, disable it since it interferes.
```bash
